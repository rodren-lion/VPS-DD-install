...
"
        ...
        if is_debian_elts; then
            if is_in_china; then
                ...
                deb_mirror=mirrors.aliyun.com/debian-elts
                initrd_mirror=mirrors.aliyun.com/debian-archive/debian
            else
                # 按道理不应该用官方源，但找不到其他源
                udeb_mirror=deb.freexian.com/extended-lts
            ...
            if is_in_china; then
                ...
                mirror=mirrors.aliyun.com/debian
            else
        ...
        # 云镜像和 firmware 下载源
        if is_in_china; then
            cdimage_mirror=https://mirrors.aliyun.com/debian-cdimage
        else
    ...
    setos_ubuntu() {
        ...
        if is_use_cloud_image; then
            ...
            if is_in_china; then
                ...
                #            https://mirrors.aliyun.com/ubuntu-cloud-images/releases/

                # mirrors.cloud.tencent.com
                ci_mirror=https://mirrors.aliyun.com/ubuntu-cloud-images
            else
    ...
    setos_arch() {
        if [ "$basearch" = "x86_64" ]; then
            if is_in_china; then
                mirror=https://mirrors.aliyun.com/archlinux
            else
                mirror=https://geo.mirror.pkgbuild.com # geoip
            fi
        else
            if is_in_china; then
                mirror=https://mirrors.aliyun.com/archlinuxarm
            else
                # https 证书有问题
    ...
    setos_nixos() {
        if is_in_china; then
            mirror=https://mirrors.aliyun.com/nix-channels
        else
    ...
    setos_gentoo() {
        if is_in_china; then
            mirror=https://mirrors.aliyun.com/gentoo
        else
...
继续 DD?'
    ...
    setos_aosc() {
        if is_in_china; then
            mirror=https://mirrors.aliyun.com/anthon/aosc-os
        else
            # 服务器在香港
    ...
    setos_centos_almalinux_rocky_fedora() {
        ...
        if is_use_cloud_image; then
            ...
            if is_in_china; then
                case $distro in
                centos) ci_mirror="https://mirrors.aliyun.com/centos-cloud/centos" ;;
                almalinux) ci_mirror="https://mirrors.aliyun.com/almalinux/$releasever/cloud/$elarch/images" ;;
                rocky) ci_mirror="https://mirrors.aliyun.com/rocky/$releasever/images/$elarch" ;;
                fedora) ci_mirror="https://mirrors.aliyun.com/fedora/releases/$releasever/Cloud/$elarch/images" ;;
                esac
            else
                case $distro in
...
install_grub_linux_efi() {
    ...
    if [ "$efi_distro" = fedora ]; then
        ...
        if is_in_china; then
            mirror=https://mirrors.aliyun.com/fedora
        else
...
download_and_extract_apk() {
    ...
    install_pkg tar xz
    is_in_china && mirror=http://mirrors.aliyun.com/alpine || mirror=https://dl-cdn.alpinelinux.org/alpine
...
install_grub_win() {
    ...
    is_in_china && grub_url=https://mirrors.aliyun.com/gnu/grub/grub-$grub_ver-for-windows.zip ||
    ...
    if is_efi; then
        ...
        if false; then
            ...
            is_in_china && host=mirrors.aliyun.com || host=deb.debian.org
